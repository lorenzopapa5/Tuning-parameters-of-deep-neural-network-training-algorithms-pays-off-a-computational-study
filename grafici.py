...
flag_def = 'opt'
tme = 3.81536
filepath = 'C:\\Users\\corra\\OneDrive\\Desktop\\PaperCNN\\Riserva\\Opt_Default_Aug1\\'
...
for nf, f in zip(list(algoritmi_8.values()),os.listdir(filepath)):
    ff = filepath+'\\'+f
    loss = grafico_loss(ff,'linear')
    algo = algoritmi_8[k]
    epochs = np.array([h for h in range(1,len(loss)+1)])
    cpu_time = epochs*tme
    ax1.plot(epochs,loss,colors_8[k],linewidth=1.75)
    ax2.plot(cpu_time, loss, colors_8[k], linewidth=1.75)
    #plt.plot(loss,colors_5[k],linewidth=1.75)
    k+=1
    labels.append(algo)

plt.legend(labels)
plt.savefig(Dataseet+'_'+network+'_'+flag_def+'allAUG.pdf')
plt.show()